...
async def _(session: CommandSession, user: u.User):
    ...
    await send_session_msg(session, get_message("plugins", __plugin_name__, 'talk_result', talk=(await talk(session, text, user)), times_left_now=dec_to_chinese(times_left_now)))
    return True
