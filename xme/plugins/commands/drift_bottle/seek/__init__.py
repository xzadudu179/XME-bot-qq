...
def is_stepping(reply, step_name="s"):
    return reply.startswith(step_name) and len(reply.split(" ")) > 1 and reply.split(" ")[1].isdigit()
...
async def _(session: CommandSession, u: user.User, validate, count_tick):
    ...
    # ---------- START ---------- #
    if str(session.event.group_id) not in enable_groups:
        if sender.is_admin:
    ...
    if (await validate()) and not is_sim:
