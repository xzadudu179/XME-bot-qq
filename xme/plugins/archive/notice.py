...
async def _(session: CommandSession):
    ...
    await send_session_msg(session, message=get_message("plugins", __plugin_name__, 'parsing', count=len(user_ids)))
    total_n_success = [0, 0]
    for user_id in user_ids:
        ...
        try:
            await session.bot.send_private_msg(user_id=user_id, message=get_message("plugins", __plugin_name__, 'message_send', message=message))
        except ActionFailed:
            print("发送失败")
            continue
        print("发送成功")
        total_n_success[1] += 1
    return await send_session_msg(session, message=get_message("plugins", __plugin_name__, 'success', percent=total_n_success[1] / total_n_success[0]))
...
async def _(session: CommandSession):
    ...
    await send_session_msg(session, get_message("plugins", __plugin_name__, 'muted'))