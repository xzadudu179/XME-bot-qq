...
async def _(session: CommandSession):
    ...
    if arg.startswith(REMOVES) and session.event.user_id in config.SUPERUSERS:
        try:
            ...
            await send_msg(session, get_message(__plugin_name__,'remove_todo_failed', arg=arg))
            return
        removed = remove_todo(index)
        await send_msg(session, get_message(__plugin_name__,'remove_todo_success', todo=removed))
        return
    elif arg and session.event.user_id in config.SUPERUSERS:
        add_todo(arg)
        await send_msg(session, get_message(__plugin_name__, 'add_todo_success', todo=arg))
        return
    elif arg:
        await send_msg(session, get_message(__plugin_name__, 'modify_todo_failed'))
        return
    lines = []
    try:
