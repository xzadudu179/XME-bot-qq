...
async def reduce_item_by_id(
    ...
    if count_str and not count_str.isdigit() and not count_str == 'all':
        if not silent:
            await send_msg(session,  try_get_msg("invalid_count", message_key, name))
    ...
    if count > user.inventory.count_item(item_get):
        if not silent:
            await send_msg(session, try_get_msg("item_count_too_many", message_key, name))
        return default
    ...
    # if user.inventory.reduce_item(item.id, count):
        if message_key:
            if not silent:
                await send_msg(session,  try_get_msg("success", message_key, name, count=count, name=item_name, pronoun=item_pronoun, **message_kwargs))
        ...
        if not silent:
            await send_msg(session,  try_get_msg("error", message_key, name, count=count, name=item_name, pronoun=item_pronoun))
...
async def reduce_item_by_index(
    ...
    if count > inv_item.count:
        if not silent:
            await send_msg(session,  try_get_msg("count_too_many", message_key, name))
        return default
    ...
    if result:
        ...
        if not silent:
            await send_msg(session,  try_get_msg("error", message_key, name, count=count, name=item_name))
    return default
