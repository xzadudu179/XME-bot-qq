...
async def get_inv_item_by_index(session, user: u.User, index_str: str, default=False, silent=False) -> InvItem | Any:
    ...
    if index_str.isdigit():
        ...
        if not silent:
            await send_session_msg(session,  get_message("plugins", __plugin_name__, cmd_name, name, "invalid_num"))
        return default

    if index >= len(user.inventory.inv_items) or index < 0:
        if not silent:
            await send_session_msg(session,  get_message("plugins", __plugin_name__, cmd_name, name, "index_out_range", num=index + 1))
        return default
    ...
    if inv_item.recorded_item is None:
        if not silent:
            await send_session_msg(session,  get_message("plugins", __plugin_name__, cmd_name, name, "no_item", num=index + 1))
        return default
    return inv_item
...
async def get_item_by_name(session, item_name: str, default=False, silent=False, fuzzy=True, threshold=80.0, user: u.User | None = None) -> Item | Any:
    ...
    if item is None:
        if not silent:
            await send_session_msg(session,  get_message("plugins", __plugin_name__, cmd_name, name, "no_name_item", name=item_name) if user is None else get_message("plugins", __plugin_name__, cmd_name, name, "user_no_item", name=item_name))
        return default
    return item
...
async def get_item_by_id(session, id_str: str, default=False, silent=False) -> Item | Any:
    ...
    if id_str.isdigit():
        ...
        if not silent:
            await send_session_msg(session,  get_message("plugins", __plugin_name__, cmd_name, name, "invalid_id"))
        return default

    item = Item.get_item(id)
    if item is None:
        if not silent:
            await send_session_msg(session,  get_message("plugins", __plugin_name__, cmd_name, name, "no_id_item", id=str(id)))
        return default
    return item