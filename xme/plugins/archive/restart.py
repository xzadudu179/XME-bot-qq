...
async def _(session: CommandSession):
    await send_session_msg(session, get_message(ic.__plugin_name__, 'restarting'))
    # await send_msg(session, "正在重启 uwu")
    try:
        subprocess.Popen(['python', './bot.py'])
        if config.DEBUG:
            await send_session_msg(session, "[DEBUG] 已启动新的 xmebot 进程啦, 正在退出原进程...")
        try:
            # os.kill(oldpid, signal.SIGTERM)
            os._exit(0)
        except Exception as ex:
            await send_session_msg(session, get_message(ic.__plugin_name__, 'restart_failed_kill', ex=ex))
            # await send_msg(session, f"无法退出原 xmebot 进程 xwx\n错误原因: {ex}")
            return
    except Exception as ex:
        await send_session_msg(session, get_message(ic.__plugin_name__, 'restart_failed', ex=ex, traceback=ex.with_traceback()))
