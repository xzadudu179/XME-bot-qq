...
def get_command_args(arg_text: str, mim_args_len: int, max_args_len: int, split_str: str = None, default: str = "", arg_types: type | list[type] = str) -> list[str]:
    """获取指令参数列表
    ...
    Args:
        ...
        arg_types (type, list[type], optional): 将参数自动转换为指定类型，设置为单个类型时将全部转为该类型. Defaults to str
    ...
    types = arg_types if isinstance(arg_types, list) else [arg_types] * len(args)
    if len(types) != len(args):
        raise ValueError(f"类型列表 {types} 长度不等于参数列表 {args}")
    type_args = [try_parse(a, types[i], default=a) for i, a in enumerate(args)]
    return type_args
...
def use_args(arg_len: int, split_str: str = None, default: str = "", arg_types: type | list[type] = str):
    ...
    Args:
        arg_len (int): 参数列表长度
        split_str (str, optional): 分割方法，默认空格. Defaults to None.
        default (str, optional): 未获取到参数时的默认值. Defaults to "".
        arg_types (type, list[type], optional): 尝试将参数自动转换为指定类型，设置为单个类型时将全部转为该类型，转换失败会变为 str. Defaults to str
    """
    def decorator(func):
        @wraps(func)
