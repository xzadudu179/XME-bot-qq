...
#     except:
#         logger.debug("无法获取撤回信息")
#         return
#     # logger.debug(session.event.group_id, session.event.user_id)
...
#         # logger.debug(c.gradient_text("#dda3f8","#66afff" ,text=user_qq_data))
...
#         # logger.debug(c.gradient_text("#dda3f8","#66afff" ,text=sender_qq_data))
...
#     # logger.debug(c.gradient_text("#dda3f8","#66afff" ,text=f"{recall_info}\n{recall_detail}"))
...
#         return await send_session_msg(session, get_message("event_parsers", "other_recalled_info",
...
#         await send_session_msg(session, get_message("event_parsers", "prevented_recall_info",
#             operator=f"[CQ:at,qq={session.event.operator_id}]",
