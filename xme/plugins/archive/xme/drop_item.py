...
async def _(session: CommandSession, user: User):
    ...
    if not args:
        await send_cmd_msg(session, "请符合指令格式输入哦 ovo\ndropitem (物品栏编号) <物品数量>")
        return
    index = int(args[0]) - 1
    count = int(args[1]) if len(args) > 1 else 1
    item = user.inventory.find_item_by_index(index)
    if not item:
        await send_cmd_msg(session, f"呜呜 没有物品栏编号为 {index + 1} 对应的物品...")
        return
    (stats, left) = user.del_item(item, count)
    if stats:
        await send_cmd_msg(session, f"{user.name}，你成功丢弃了 {count} 个 \"{item.name}\"哦 owo")
    else:
        await send_cmd_msg(session, f"{user.name}: 丢弃物品 \"{item.name}\" *{count} 失败 xwx\n还需要 {left} 个物品")