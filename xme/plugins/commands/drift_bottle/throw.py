...
async def _(session: CommandSession):
    ...
    if not arg:
        await send_session_msg(session, get_message(__plugin_name__, "nothing_to_throw", command_name=f"{config.COMMAND_START[0]}{command_name}"))
        # await send_msg(session, f"漂流瓶似乎没有内容呢ovo\n格式：\n{config.COMMAND_START[0]}throw (漂流瓶内容)")
        return
    if len(arg) > MAX_LENGTH:
        await send_session_msg(session, get_message(__plugin_name__, "content_too_many", max_length=MAX_LENGTH))
        # await send_msg(session, f"瓶子的内容太多啦！要 200 字以内哦")
        return
    if arg.count('\n') > MAX_LINES or arg.count('\r') > MAX_LINES:
        await send_session_msg(session, get_message(__plugin_name__, "lines_too_many", max_lines=MAX_LINES))
    ...
    for k, bottle in bottles_dict['bottles'].items():
        ...
        # if arg == bottle['content']:
            await send_session_msg(session, get_message(__plugin_name__, "content_already_thrown", content=bottle['content'], id=k))
    ...
    await send_session_msg(session, get_message(__plugin_name__, 'throwed', id=id))
